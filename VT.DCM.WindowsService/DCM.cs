...
{
    public partial class DCM : ServiceBase
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(DCM));
        ...
        protected override void OnStop()
        {
            ...
            DeviceHelper dHelper = DeviceHelper.Instance();
            ICollection<string> keys = dHelper.DeviceList.Keys;
            List<string> keysLoop = new List<string>();
            foreach (string key in keys)
            {
                keysLoop.Add(key);
            }
            foreach (string key in keysLoop)
            {
                JBODevice jboDevice = dHelper.DeviceList[key];
                if (jboDevice != null)
                {
                    jboDevice.Close();
                    dHelper.DeviceList.Remove(key);
                }
            }
        ...
        private void ProcessOneMessage(object tcp)
        {
            ...
            while (!exitWhile)
            {
                ...
                foreach (Message message in messageList)
                {
                    ...
                    DeviceHelper dHelper = DeviceHelper.Instance();
                    if (!dHelper.DeviceList.ContainsKey(idcode))
                    {
                        lock (obj)
                        {
                            if (!dHelper.DeviceList.ContainsKey(idcode))
                            {
                                ...
                                dHelper.DeviceList[idcode] = jboDevice;
                            }
                        }
                    }
