...
namespace VT.DCM.WindowsService
{
    public class MessageHandler
    {
        ...
        //[begin]更新星历数据需要的字段
        private static readonly string ephemerisPath = ConfigurationManager.AppSettings["EphemerisPath"]; //Environment.CurrentDirectory + "\\ephemeris.ee";
        ...
        public void SendAgpsCheckResult(byte[] messageBuffer, int messageBufferSize)
        {
            ...
            log.Debug(string.Format("-> Device last update date [{0}]", dtLastUpdateTime.ToString("yyyy-MM-dd HH:mm:ss")));
            log.Debug(string.Format("-> File update date [{0}]", dtFileUpdateTime.ToString("yyyy-MM-dd HH:mm:ss")));
            log.Debug(string.Format("-> Need update [{0}]", needUpdate));
